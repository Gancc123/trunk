...
const RC CreateFile(const char *fileName)//创建分页文件
{
	...
	bitmap[0]|=0x01;
	if(_lseek(fd,0,SEEK_SET)==-1)/* off_t lseek(int handle, off_t offset, int fromwhere);
...
PF_FileHandle * getPF_FileHandle(void )//获得分页文件句柄
{
...
const RC openFile(char *fileName,PF_FileHandle *fileHandle)//打开分页文件
{
	int fd;
	PF_FileHandle *pfilehandle=fileHandle;
	RC tmp;
	if((fd=_open(fileName,O_RDWR|_O_BINARY))<0)//打开文件
		return PF_FILEERR;
	pfilehandle->bopen=true;
	pfilehandle->fileName=fileName;
	pfilehandle->fileDesc=fd;//更新句柄
	if((tmp=AllocateBlock(&pfilehandle->pHdrFrame))!=SUCCESS){//在缓冲区分配一帧
		_close(fd);
		return tmp;
	}
...
const RC AllocateBlock(Frame **buffer)//在页面缓冲区中分配一帧
{
	int i,min,offset;
	bool flag;
	clock_t mintime;
	for(i=0;i<PF_BUFFER_SIZE;i++)
		if(bf_manager.allocated[i]==false){//还有空闲
